...
(define apply-env ; Env x Var -> Expval
  (lambda (env target-var)
    (cases environ env
      ...
      [extend-env-rec [p-name p-params p-body saved-env] (cond
        [(equal? p-name target-var) (let ([ref (newref! (proc-val p-params p-body
            (extend-env-rec p-name p-params p-body saved-env)))])
            (add-letrec-proc-ref! ref)
            ref  
          )
        ]
        [else (apply-env saved-env target-var)]
      )]
)))
...
(define env-for-each 
  (lambda [env func] 
    (cases environ env
      [empty-env () (unit-val)]
      [extend-env (var val env*)
        (func val) (env-for-each env* func)]
      [extend-env-rec (p-name p-params p-body saved-env)
        (env-for-each saved-env func)]
    )
))

(define increment-all
  (lambda [env]
    (env-for-each env increment_count!)
))
  
(define decrement-all
  (lambda [env]
    (env-for-each env decrement_count!)
))

;; ================ Escaped References ==================================

(define escaped-references! '())

;; Adds a reference that has currently escaped
(define add-escaped-reference! (lambda (ev)
  (cases expval ev
    [ref-val (ref) (increment_count! ev) (set! escaped-references! (cons ev escaped-references!))]
    [else (raise-exception 'add-escaped-value "Expressed value is not a number or a Boolean: ~s" ev)]
  )
))

;; Decrements all reference counts in escaped-refs
(define decrement-all-in-list* (lambda (ls)
  (cond
    [(null? ls) 'donothing]
    [else (decrement_count! (car ls)) (decrement-all-in-list* (cdr ls))]
  )
))

;; Flushes all the values in escaped references
(define flush-escaped-references! (lambda ()
  (decrement-all-in-list* escaped-references!)
  (set! escaped-references! '())
))

(define letrec-proc-refs! '())

;; Adds a reference that has been created on behalf of a letrec proc body
(define add-letrec-proc-ref! (lambda (ev)
  (cases expval ev
    [ref-val (ref) (set! letrec-proc-refs! (cons ev letrec-proc-refs!))]
    [else (raise-exception 'add-letrec-proc "Expressed value is not a number or a Boolean: ~s" ev)]
  )
))

;; Flushes all the values in letrec proc references
(define flush-letrec-proc-refs! (lambda ()
  (decrement-all-in-list* letrec-proc-refs!)
  (set! letrec-proc-refs! '())
))
...
(define value-of
  (lambda (prog env)
    ...
    (cases program prog
      ...
      [def-prog (var exp) (increment-all env) (cons (unit-val) (extend-env var (newref! (value-of-exp exp env)) env))]
      [else (raise-exception 'value-of-prog "Abstract syntax case not implemented: ~s" (car prog))])
    ])
    ;; flush any references that escaped during the evaluation of the program
    (flush-escaped-references!)
    res
)))
...
(define value-of-exp
  (lambda (exp env)
    (cases expression exp
      ...
      [let-exp (var exp1 exp2) (let ([ref (newref! (value-of-exp exp1 env))])
        (let ([res (value-of-exp exp2 (extend-env var ref env))])
          (display "During execution: ")
          (display-store the-store!)
          (newline)
          (cases expval res
            ; The proc-val returning may use the variable from this let statement
            [proc-val (params body saved-env) (add-escaped-reference! ref)]
            ; If we are returning a ref-val, see if it the one used in this let statement. Add it to escaped references if so
            [ref-val (retRef) (let 
              ([letRef (cases expval ref
                [ref-val (r) r]
                [else -1]
              )])
              (if (= retRef letRef) (add-escaped-reference! ref))
            )]
            [else 'donothing]
          )
          (decrement_count! ref)
      ...
      ;; ======== HW 6 =============
      ;; go through body and find all var-exps whenever one is found check if it is vars. If it is then it a parameter else increment the reference count.
                ...
                [proc-val
                 ...
                 (cond
                  [(= (length params) (length rands))
                    (let ([proc-env (fold-left
                      (lambda (acc param exp)
                        (extend-env param
                          (cases expression exp
                            [var-exp (var) 
                            (let [[ref (apply-env env var)]] 
                            (increment_count! ref) ref)]   
                            ;; increment reference count for vars only
                            [else (newref! (value-of-exp exp env))])
                        acc))
                    saved-env
                    params
                    rands)])
                    (let ([ret (value-of-exp body proc-env)])
                    ...
                    (fold-left
                      (lambda (acc param exp)
                          (cases expression exp
                            [var-exp (var) 
                              (let [[ref (apply-env env var)]] 
                                (decrement_count! ref) acc)]   
                                ;; decrement reference count for vars only
                            [else (decrement_count! (apply-env proc-env param))])
                        )
                    (unit-val)
                    params
                    rands)
                  ...
                  ;; decrement the reference count after body
      ...
      ;; need to iterate through p-vars (arbno) and make multiple references: map
      [letrec-exp (p-name p-vars p-body body) (let ([res (value-of-exp body (extend-env-rec p-name p-vars p-body env))])
        (flush-letrec-proc-refs!)
        res
      )]  
...
(define start
  (lambda ()
    (begin
	(display "       ________________   ___/-\\___     ___/-\\___     ___/-\\___")
  (newline)
	(display "     / /             ||  |---------|   |---------|   |---------|")
  (newline)
	(display "    / /              ||   |       |     | | | | |     |   |   |")
  (newline)
	(display "   / /             __||   |       |     | | | | |     | | | | |")
  (newline)
	(display "  / /    \\        I  ||   | JAMES |     |  DANI |     | MANAV |")
  (newline)
	(display " (-------------------||   | | | | |     | | | | |     | | | | |")
  (newline)
	(display " ||               == ||   |_______|     |_______|     |_______|")
  (newline)
	(display " || CSC370 TRASH CO  | =============================================")
  (newline)
	(display " ||          ____    |                                ____      |")
  (newline)
	(display "( | o      / ____ \\                                 / ____ \\    |)")
  (newline)
	(display " ||      / / . . \\ \\                              / / . . \\ \\   |")
  (newline)
	(display "[ |_____| | .   . | |____________________________| | .   . | |__]")
  (newline)
	(display "          | .   . |                                | .   . |")
  (newline)
	(display "           \\_____/                                  \\_____/")
