...
(define value-of-exp
  (lambda (exp env)
    (cases expression exp
      ...
      ;; Pass by name.
      [var-exp (var)
        (let [[ev (deref (apply-env env var))]]
          (cases expval ev
            [a-thunk (exp* env*)
              (setref! (apply-env env var) (value-of-exp exp* env*)) ;; Update store
              (deref (apply-env env var))] ;; Return updated result.
            [else ev]))]
            ...
            [proc-val [param body saved-env]
              (value-of-exp body
                (cases expression rand
                  ...
                  [else
                    (extend-env param (newref! (a-thunk rand env)) saved-env)]))] ;;By name.
            [else
              (raise-exception
                'value-of-exp
