...
(define env->string*
  (lambda (env)
	...
	(string-append ", " (symbol->string var) 
		       ...
		       (env->string* env*))]
      [extend-env-rec (p-name p-param p-body saved-env)
	(string-append ", " (symbol->string p-name) "(" (number->string (length p-param)) ")"
		       (env->string* saved-env))])))
  ...
  (lambda (ev)
    (cases expval ev
           ...
           [proc-val (param body saved-env)
                     (string-append "#proc(" (number->string (length param)) ")")] 
           [unit-val () ""]
    ...
    (cases expression exp
      ...
      [call-exp [rator rand]
        (let [[ev1 (value-of-exp rator env)]
              [ev2 (map (lambda [r] (value-of-exp r env)) rand)]]   
          (cases expval ev1
            [proc-val [param body saved-env]
            (if (= (length param) (length ev2))
              (value-of-exp body (fold-left (lambda [acc e] (extend-env (car e) (cdr e) acc)) env (map cons  param ev2)))
              (raise-exception
                'call-exp
                "Incorrrect number of arguments ~s" rator))]
            [else
              (raise-exception
                'value-of-exp
      ...
      ;;(display (env->string (extend-env-rec p-name p-param p-body env)))
      ...
      [block-exp (exp) (fold-left (lambda [acc l] (value-of-exp l env)) (unit-val) exp)]
